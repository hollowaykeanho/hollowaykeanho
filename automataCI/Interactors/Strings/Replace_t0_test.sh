...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_Strings_Replace

TEST ID    :
t0

DESCRIPTION:
Function can handle valid string.
"
...
____target="${PWD}/interactors/Strings/Replace.sh"
...
. "$____target"
command -V Interactors_Strings_Replace > /dev/null
if [ $? -ne 0 ]; then
...
____target="你你a"
____candidate="他他X"
____expect="e你feeeff他他XergaegE你F"
____output="$(Interactors_Strings_Replace \
        "$____input" \
        "$____target" \
        "$____candidate" \
)"
...
1>&2 printf --  "Given candidate sample :\n|%s|\n\n" "$____candidate"
