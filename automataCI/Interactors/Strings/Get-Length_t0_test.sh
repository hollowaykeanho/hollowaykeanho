...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_Strings_Get_Length
...
____target="${PWD}/interactors/Strings/Get-Length.sh"
...
. "$____target"
command -V Interactors_Strings_Get_Length > /dev/null
if [ $? -ne 0 ]; then
...
____expect="20"
____output="$(Interactors_Strings_Get_Length "$____input")"
____process=$?
