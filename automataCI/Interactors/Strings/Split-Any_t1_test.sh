...
____target="${PWD}/interactors/Strings/Split-Any.sh"
...
. "$____target"
command -V Interactors_Strings_Split_Any > /dev/null
if [ $? -ne 0 ]; then
...
feeeff
e"
____output="$(Interactors_Strings_Split_Any \
        "$____input" \
        "$____target" \
        "-1" \
        "-1" \
