...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_Booleans_Validate_Boolean
...
____target="${PWD}/interactors/Booleans/Validate-Boolean.sh"
...
. "$____target"
command -V Interactors_Booleans_Validate_Boolean > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_Booleans_Validate_Boolean "$____input")"
____process=$?
