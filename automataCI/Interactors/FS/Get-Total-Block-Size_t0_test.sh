...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_FS_Get_Total_Block_Size
...
. "$____target"
command -V Interactors_FS_Get_Total_Block_Size > /dev/null
if [ $? -ne 0 ]; then
...
____expect=""
____output="$(Interactors_FS_Get_Total_Block_Size "$____input_path")"
____process=$?
