...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_FS_Is_Symbolic_Link
...
____target="${PWD}/interactors/FS/Is-Symbolic-Link.sh"
...
. "$____target"
command -V Interactors_FS_Is_Symbolic_Link > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_FS_Is_Symbolic_Link "$____input_dest")"
____process=$?
