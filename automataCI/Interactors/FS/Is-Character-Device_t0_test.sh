...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_FS_Is_Character_Device
...
____target="${PWD}/interactors/FS/Is-Character-Device.sh"
...
. "$____target"
command -V Interactors_FS_Is_Character_Device > /dev/null
if [ $? -ne 0 ]; then
...
____input="/dev/tty0"
____output="$(Interactors_FS_Is_Character_Device "$____input")"
____process=$?
