...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_FS_Get_Model_From_Block_Device
...
____target="${PWD}/interactors/FS/Get-Model-From-Block-Device.sh"
...
. "$____target"
command -V Interactors_FS_Get_Model_From_Block_Device > /dev/null
if [ $? -ne 0 ]; then
...
____expect=""
____output="$(Interactors_FS_Get_Model_From_Block_Device "$____input_path")"
____process=$?
