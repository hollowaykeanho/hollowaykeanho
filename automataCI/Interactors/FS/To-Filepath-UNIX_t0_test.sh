...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_FS_To_Filepath_UNIX
...
____target="${PWD}/interactors/FS/To-Filepath-UNIX.sh"
...
. "$____target"
command -V Interactors_FS_To_Filepath_UNIX > /dev/null
if [ $? -ne 0 ]; then
...
____expect="/path/to/text/test_file.txt"

____output="$(HestiaFS_To_Filepath_UNIX "$____input")"
____process=$?
