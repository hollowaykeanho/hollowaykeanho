...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_FS_List_Partitions_From_Block_Device
...
____target="${PWD}/interactors/FS/List-Partitions-From-Block-Device.sh"
...
. "$____target"
command -V Interactors_FS_List_Partitions_From_Block_Device > /dev/null
if [ $? -ne 0 ]; then
...
____input="/dev/sda"
____output="$(Interactors_FS_List_Partitions_From_Block_Device "$____input")"
____process=$?
