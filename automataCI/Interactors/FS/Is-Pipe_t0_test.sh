...
____target="${PWD}/interactors/FS/Is-Pipe.sh"
...
. "$____target"
command -V Interactors_FS_Is_Pipe > /dev/null
if [ $? -ne 0 ]; then
...
____input="./test-pipe"
rm -rf "$____input" 2> /dev/null
mkfifo "$____input"

____output="$(HestiaFS_Is_Pipe "$____input")"
____process=$?
