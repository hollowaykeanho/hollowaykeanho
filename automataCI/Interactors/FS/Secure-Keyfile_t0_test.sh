...
____target="${PWD}/interactors/FS/Secure-Keyfile.sh"
...
. "$____target"
command -V Interactors_FS_Secure_Keyfile > /dev/null
if [ $? -ne 0 ]; then
...
____output="$( \
        Interactors_FS_Secure_Keyfile "$____input_file" \
)"
____process=$?
