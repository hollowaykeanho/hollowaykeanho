...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_FS_Is_Filepath

TEST ID    :
t0

DESCRIPTION:
Function can run properly with:
        1. Valid Path (file)
"
...
____target="${PWD}/interactors/FS/Is-Filepath.sh"
...
. "$____target"
command -V Interactors_FS_Is_Filepath > /dev/null
if [ $? -ne 0 ]; then
...
____input="/this/is_a@bpath/here"
____expect="[NON-EMPTY]"
____output="$(Interactors_FS_Is_Filepath "$____input")"
____process=$?
