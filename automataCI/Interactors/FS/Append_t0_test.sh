...
____target="${PWD}/interactors/FS/Append.sh"
...
. "$____target"
command -V Interactors_FS_Append > /dev/null
if [ $? -ne 0 ]; then
...
____target="${PWD}/interactors/FS/Write.sh"
1>&2 printf --  "Checking '%s'...\n" "$____target"
if [ ! -f "$____target" ]; then
        1>&2 printf --  "[ FAILED ] missing file!\n"
        exit 1
...
____input_existing="ERheshseths
sehser
esbsbsbsbs
"
____input_content="Hello World
"
____expect="${____input_existing}${____input_content}"
rm -rf "$____input_file" 2> /dev/null

Interactors_FS_Write "$____input_file" "$____input_existing"
____process_write=$?

Interactors_FS_Append "$____input_file" "$____input_content"
____process=$?
...
1>&2 printf --  "Given input (existing) :\n|%s|\n\n" "$____input_existing"
