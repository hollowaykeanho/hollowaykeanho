...
____target="${PWD}/interactors/FS/Is-Mounted-To.sh"
...
. "$____target"
command -V Interactors_FS_Is_Mounted_To > /dev/null
if [ $? -ne 0 ]; then
...
____input_source="proc"
____input_dest="/proc"
____output="$(Interactors_FS_Is_Mounted_To \
        "$____input_dest" \
        "$____input_source" \
)"
____process=$?
