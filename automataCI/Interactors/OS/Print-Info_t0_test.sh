...
____target="${PWD}/interactors/OS/Print-Info.sh"
...
. "$____target"
command -V Interactors_OS_Print_Info > /dev/null
if [ $? -ne 0 ]; then
...
"
____output="$(Interactors_OS_Print_Info "$____input" 2>&1)"
____process=$?
1>&2 printf --  "%s" "\
Given sample:
|${____input}|
