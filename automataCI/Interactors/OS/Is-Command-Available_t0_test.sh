...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_OS_Is_Command_Available
...
____target="${PWD}/interactors/OS/Is-Command-Available.sh"
...
. "$____target"
command -V Interactors_OS_Is_Command_Available > /dev/null
if [ $? -ne 0 ]; then
...
____input="dd"
unset "$____input"
____output="$(Interactors_OS_Is_Command_Available "$____input")"
____process=$?
