...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_OS_Is_Command_External
...
____target="${PWD}/interactors/OS/Is-Command-External.sh"
...
. "$____target"
command -V Interactors_OS_Is_Command_External > /dev/null
if [ $? -ne 0 ]; then
...
____input="dd"
unset "$____input"
____output="$(Interactors_OS_Is_Command_External "$____input")"
____process=$?
