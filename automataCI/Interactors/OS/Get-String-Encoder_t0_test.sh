...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_OS_Get_String_Encoder
...
____target="${PWD}/interactors/OS/Get-String-Encoder.sh"
...
. "$____target"
command -V Interactors_OS_Get_String_Encoder > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_OS_Get_String_Encoder)"
____process=$?
