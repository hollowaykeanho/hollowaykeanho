...
____target="${PWD}/interactors/OS/Print-Success.sh"
...
. "$____target"
command -V Interactors_OS_Print_Success > /dev/null
if [ $? -ne 0 ]; then
...
"
____output="$(Interactors_OS_Print_Success "$____input" 2>&1)"
____process=$?
1>&2 printf --  "%s" "\
Given sample:
|${____input}|
...
IMPORTANT NOTE
This is a graphical design function. Manual reviews is required.
