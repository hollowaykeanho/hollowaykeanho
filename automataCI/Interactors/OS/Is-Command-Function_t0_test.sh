...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_OS_Is_Command_Function
...
____target="${PWD}/interactors/OS/Is-Command-Function.sh"
...
. "$____target"
command -V Interactors_OS_Is_Command_Function > /dev/null
if [ $? -ne 0 ]; then
...
whatever_this_long_name_to_avoid_conflict() {
        :
}
____input="whatever_this_long_name_to_avoid_conflict"
____output="$(Interactors_OS_Is_Command_Function "$____input")"
____process=$?
unset "$____input"
