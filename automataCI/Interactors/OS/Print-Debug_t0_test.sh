...
____target="${PWD}/interactors/OS/Print-Debug.sh"
...
. "$____target"
command -V Interactors_OS_Print_Debug > /dev/null
if [ $? -ne 0 ]; then
...
"
____output="$(HestiaOS_Print_Debug "$____input" 2>&1)"
____process=$?
1>&2 printf --  "%s" "\
Given sample:
|${____input}|
