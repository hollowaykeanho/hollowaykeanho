...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_OS_Is_Command_Alias
...
____target="${PWD}/interactors/OS/Is-Command-Alias.sh"
...
. "$____target"
command -V Interactors_OS_Is_Command_Alias > /dev/null
if [ $? -ne 0 ]; then
...
alias "$____input"="ls"
____output="$(Interactors_OS_Is_Command_Alias "$____input")"
____process=$?
unset "$____input"
