...
____target="${PWD}/interactors/OS/Get-Machine-ID.sh"
...
. "$____target"
command -V Interactors_OS_Get_Machine_ID > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_OS_Get_Machine_ID)"
____process=$?
