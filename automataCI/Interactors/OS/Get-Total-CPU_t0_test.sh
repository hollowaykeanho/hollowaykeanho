...
____target="${PWD}/interactors/OS/Get-Total-CPU.sh"
...
. "$____target"
command -V Interactors_OS_Get_Total_CPU > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_OS_Get_Total_CPU)"
____process=$?
...
if [ ! "$____output" = "" ] && [ "$____output" -ge 1 ]; then
        1>&2 printf --  "[ PASSED ]\n"
        exit 0
fi
