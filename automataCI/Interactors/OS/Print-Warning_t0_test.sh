...
____target="${PWD}/interactors/OS/Print-Warning.sh"
...
. "$____target"
command -V Interactors_OS_Print_Warning > /dev/null
if [ $? -ne 0 ]; then
...
"
____output="$(Interactors_OS_Print_Warning "$____input" 2>&1)"
____process=$?
1>&2 printf --  "%s" "\
Given sample:
|${____input}|
