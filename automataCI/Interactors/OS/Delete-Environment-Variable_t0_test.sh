...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_OS_Delete_Environment_Variable
...
____target="${PWD}/interactors/OS/Delete-Environment-Variable.sh"
...
. "$____target"
command -V Interactors_OS_Delete_Environment_Variable > /dev/null
if [ $? -ne 0 ]; then
...
____value="before"
____expect=""
export "$____input_key" "$____value"
Interactors_OS_Delete_Environment_Variable "$____input_key"
____process=$?
____output="$MY_VAR"
1>&2 printf --  "Given sample key          :\n|%s|\n\n" "$____input_key"
1>&2 printf --  "Given sample value        :\n|%s|\n\n" "$____value"
