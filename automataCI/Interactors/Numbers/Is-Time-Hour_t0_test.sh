...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_Numbers_Is_Time_Hour
...
____target="${PWD}/interactors/Numbers/Is-Time-Hour.sh"
...
. "$____target"
command -V Interactors_Numbers_Is_Time_Hour > /dev/null
if [ $? -ne 0 ]; then
...
____input="12"
____input_format="true"
____expect="[NON EMPTY]"
____output="$(Interactors_Numbers_Is_Time_Hour "$____input")"
____process=$?
1>&2 printf -- "%s" "\
Given input:
|${____input}|

Given input format (not feeded):
|${____input_format}|
