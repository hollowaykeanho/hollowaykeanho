...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_Numbers_Is_Calendar_Day
...
____target="${PWD}/interactors/Numbers/Is-Calendar-Day.sh"
...
. "$____target"
command -V Interactors_Numbers_Is_Calendar_Day > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_Numbers_Is_Calendar_Day "$____input")"
____process=$?
1>&2 printf -- "%s" "\
Given input:
|${____input}|
