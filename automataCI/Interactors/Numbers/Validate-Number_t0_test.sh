...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_Numbers_Validate_Number
...
____target="${PWD}/interactors/Numbers/Validate-Number.sh"
...
. "$____target"
command -V Interactors_Numbers_Validate_Number > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_Numbers_Validate_Number "$____input")"
____process=$?
1>&2 printf -- "%s" "\
Given input:
|${____input}|
