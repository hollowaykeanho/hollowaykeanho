...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_Numbers_Is_Time_Minute
...
____target="${PWD}/interactors/Numbers/Is-Time-Minute.sh"
...
. "$____target"
command -V Interactors_Numbers_Is_Time_Minute > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_Numbers_Is_Time_Minute "$____input")"
____process=$?
1>&2 printf -- "%s" "\
Given input:
|${____input}|
