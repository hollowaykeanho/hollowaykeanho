...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_Numbers_Is_Time_Second
...
____target="${PWD}/interactors/Numbers/Is-Time-Second.sh"
...
. "$____target"
command -V Interactors_Numbers_Is_Time_Second > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_Numbers_Is_Time_Second "$____input")"
____process=$?
1>&2 printf -- "%s" "\
Given input:
|${____input}|
