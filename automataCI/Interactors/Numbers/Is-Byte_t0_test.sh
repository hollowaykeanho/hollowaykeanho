...
____target="${PWD}/interactors/Numbers/Is-Byte.sh"
...
. "$____target"
command -V Interactors_Numbers_Is_Byte > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_Numbers_Is_Byte "$____input")"
____process=$?
1>&2 printf -- "%s" "\
Given input:
|${____input}|
