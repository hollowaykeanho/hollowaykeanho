...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_Numbers_Is_Negative
...
____target="${PWD}/interactors/Numbers/Is-Negative.sh"
...
. "$____target"
command -V Interactors_Numbers_Is_Negative > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_Numbers_Is_Negative "$____input")"
____process=$?
1>&2 printf -- "%s" "\
Given input:
|${____input}|
