...
1>&2 printf --  "%s\n" "\
TEST CASE  :
Interactors_Numbers_Is_Positive
...
____target="${PWD}/interactors/Numbers/Is-Positive.sh"
...
. "$____target"
command -V Interactors_Numbers_Is_Positive > /dev/null
if [ $? -ne 0 ]; then
...
____output="$(Interactors_Numbers_Is_Positive "$____input")"
____process=$?
1>&2 printf -- "%s" "\
Given input:
|${____input}|
