...
HestiaFS_Get_Serial_Number_From_Block_Device() {
        #____device_label="$1"
        ...
        if [ $(HestiaFS_Is_Exist "/sys/class/block/${1##*/}/device/serial") -eq $HestiaSIGNALS_OK ]; then
                ____had_read=false
                ____old_IFS="$IFS"
                while IFS="" read -r ____line || [ -n "$____line" ]; do
                        ...
                        printf -- "%s" "$____line"
                        break
                done < "/sys/class/block/${1##*/}/device/serial"
                IFS="$____old_IFS"
