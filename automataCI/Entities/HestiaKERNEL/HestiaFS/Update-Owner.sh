...
HestiaFS_Update_Owner() {
        ...
        if [ "$(HestiaOS_Is_Command_Available "chown")" -ne $HestiaSIGNALS_OK ]; then
        ...
        if [ ! "$3" = "" ] &&
        [ "$(HestiaNUMBERS_Is_Positive "$3")" -ne $HestiaSIGNALS_OK ]; then
                return $HestiaSIGNALS_DATA_INVALID
        fi
