...
HestiaUNICODES_Wrap_Left() {
        ...
        while [ ! "$____content_unicode" = "" ]; do
                ...
                if [ "$____count" -ge "$2" ]; then
                        if [  "$(HestiaUNICODES_Is_Whitespace "$____char")" -eq $HestiaSIGNALS_OK ]; then
                                ...
                                if [ ! "$____before" = "" ]; then
                                        ...
                                        ____holding_unicode="${____holding_unicode}${____before}, "
                                fi

                                if [ ! "$____cut" = "" ]; then
                                        ____before="${____holding_unicode%", "}"
                                        ____before="${____before##*", "}"
                                        if [  "$(HestiaUNICODES_Is_Whitespace "$____before")" -ne $HestiaSIGNALS_OK ] &&
                                        [  "$(HestiaUNICODES_Is_Punctuation "$____before")" -ne $HestiaSIGNALS_OK ]; then

                                                ____cut="${____cut%, }"
                                                ____holding_unicode="${____holding_unicode}${____cut}, "
                                        fi
                                fi
                                ...
                                if [ ! "$____after" = "" ]; then
                                        ____after="${____after%, }"
                                        ____holding_unicode="${____holding_unicode}${____after}, "
                                fi
                                ...
                                # (e.g. 1 char width)
                                if [ "$____count" -ge "$2" ]; then
                                        if [ ! "$____holding_unicode" = "" ]; then
                                                ____holding_unicode="${____holding_unicode%", "}"
                                                ____cut="${____holding_unicode##*", "}"
                                                if [  "$(HestiaUNICODES_Is_Whitespace "$____cut")" -eq $HestiaSIGNALS_OK ]; then
                                                ...
                                                ____after="10, ${____after}${____char}, "
                                        else
                                                ____after="${____after}10, ${____char}, "
                                        fi
                                ...
                                # some other characters - transact as usual
                                if [ ! "$____before" = "" ]; then
                                        ____before="${____before%, }"
                                        ____holding_unicode="${____holding_unicode}${____before}, "
                                fi
                ...
                [ "$(HestiaUNICODES_Is_Whitespace "$____char")" -eq $HestiaSIGNALS_OK ]; then
                        if [ ! "$____before" = "" ]; then
                                # push buffer_before into output register
                                ____before="${____before%, }"
                                ____holding_unicode="${____holding_unicode}${____before}, "
