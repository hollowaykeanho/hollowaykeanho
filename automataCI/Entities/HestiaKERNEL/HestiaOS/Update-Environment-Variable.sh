...
HestiaOS_Update_Environment_Variable() {
        ...
        # validate input
        case "$(HestiaOS_Is_Environment_Variable "$1" "$2")" in
        $HestiaSIGNALS_OK)
                ;;
        ...
        *)
        ...
        # execute
        ## It's OK to use 'export' rather than the conventional and dangerous
        ## 'exec' since:
        ##      (1) by normal execution: there variables are not exposed to
        ##                               parent.
        ##      (2) by source execution: normal defined (without export) are
        ##                               delivered to parent anyway.
        export "$1"="$2"
        if [ $? -ne 0 ]; then
